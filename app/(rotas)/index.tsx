...
export default function TabOneScreen() {
  ...
  const pegaMaisCards = async () => {
    axios
      .get(process.env.EXPO_PUBLIC_API_URL + `cartao?page=${page + 1}&size=10`)
      .then((res) => {
        setPage(page + 1);
  ...
  const pegarCards = async () => {
    axios
      .get(process.env.EXPO_PUBLIC_API_URL + "cartao?page=0&size=10")
      .then((res) => {
  ...
  return (
    <View>
      ...
      >
        <Input
          ...
          onChange={(e: NativeSyntheticEvent<TextInputChangeEventData>) => {
            setThingToSay(e.nativeEvent.text);
      ...
      <ScrollView
        onMomentumScrollEnd={pegaMaisCards}
